...
func main() {
	...
	router.GET("/bmkg", CreateBMKG)
	router.GET("/nodejs", CustomerBank)
...
type ResponseBMKG struct {
	Status  int    `json:"status"`
	Message string `json:"message"`
	Data    []Infogempa
}

func CreateBMKG(c *gin.Context) {
	var td Todo
	if err := c.ShouldBindJSON(&td); err != nil {
		c.JSON(http.StatusUnprocessableEntity, "invalid json")
		return
...
func CustomerBank(c *gin.Context) {
	var td Todo
	if err := c.ShouldBindJSON(&td); err != nil {
		c.JSON(http.StatusUnprocessableEntity, "invalid json")
		return
...
type Customer struct {
	...
	Response struct {
		CustomerID uint64 `json:"cust_id"`
...
//https://youtu.be/Sc8uk8apgFM?t=14732
